...
class signupView extends React.Component {
  constructor(props) {
    ...
    console.log("Signup page", this.props.match.params.userAlias);
  }
  ...
  loginProcess() {
    if (this.state.userAlias.trim() != "" && this.state.userAlias.trim() != null && this.state.userAliasValid == true) {
      ...
      console.log("Wrong data", this.state.userAlias);
      ...
      <Grid>
        <Row className="show-grid">
          <Col xs={12} md={12} className="viewPageTitle">
            <div className="viewPageTitleYear">Signup to Kaleidoscope</div>
            <input
            ...
            <Button className="buttonClick"
              onClick={async () => {
                const response = await this.loginProcess();
              }}
            >Login</Button>
          </Col>
        </Row>
